...
class VoiceEmailManager:
    ...
    def fetch_emails(self, folder="inbox", unseen=True):
        try:
            ...
            email_list = {}
            for idx, email_id in enumerate(email_ids[:40], start=1):
    ...
    def summarize_email(self, email_msg):
        ...
        full_body = self._get_email_body(email_msg)['text'].strip()
        ...
        try:
            ...
            if len(full_body) > 1024:
                ...
                for chunk in chunks:
                    summary = self.summarizer(
                        full_body,
                        max_length=100,  # Optimal for business emails
                        min_length=50,
                        do_sample=False,  # Prevents random additions
                        truncation=True  # Ensures clean handling of long text
                    )[0]["summary_text"]
                    summaries.append(summary)
                full_summary = ' '.join(summaries)
                # Post-process the combined summary to improve coherence
            ...
            # Clean up text
            full_summary = re.sub(r'\s([,.!?])', r'\1', full_summary)  # Fix spaces before punctuation
            full_summary = re.sub(r'\s+', ' ', full_summary).strip()

            # Ensure the summary ends with proper punctuation
            if not full_summary[-1] in {'.', '!', '?'}:
                full_summary += '...'
    ...
    def _handle_email_search(self, initial_query=None):
        ...
        for idx, (email_id, msg) in results:  # Fixed unpacking here
        ...
        while True:
            ...
            if num in dict(results):
                email_id, msg = dict(results)[num]
                body_data = self._get_email_body(msg)
                if body_data and 'text' in body_data:
                    self.speak(body_data['text'])  # Only speak the extracted text
                else:
                    self.speak("Could not read email body")

    def search_emails(self, query):
        """
        Search through ALL emails (read and unread) for matching results.

        Args:
            query (str): Search term to look for in subject/sender/body.
        Returns:
            list: Tuples of (index, (email_id, email_message)) for matches.
        """
        # Fetch ALL emails (including read ones)
        emails_dict = self.fetch_emails(unseen=False)
        results = []
        ...
        for idx, (email_id, msg) in emails_dict.items():
            try:
                # Decode subject (handles encoded email headers)
                ...
                # Get body text (using your existing _get_email_body method)
                body_data = self._get_email_body(msg)
                body = body_data.get('text', '').lower() if body_data else ''

                # Search in subject, sender, or body
                if (query in subject) or (query in sender) or (query in body):
                    results.append((idx, (email_id, msg)))

            except Exception as e:
                print(f"Error processing email {idx}: {str(e)}")
                continue

        # Sort by most recent first (using email Date header)
        results.sort(
            key=lambda x: email.utils.parsedate_to_datetime(x[1][1].get('date', '1970-01-01')),
            reverse=True
        )
